...
const AddEditUserModal: React.FC<{
    ...
    const isJudgeOrCoordinatorToEdit = useMemo(() => {
        return !!userToEdit && (userToEdit.roles.includes(UserRole.JUDGE) || userToEdit.roles.includes(UserRole.COORDINATOR));
    }, [userToEdit]);
...
export const UserManagementPage: React.FC = () => {
    ...
    const activeJudgeIdsForLevel = useMemo(() => {
        ...
        const projectsForAssignmentsIds = new Set(projectsForAssignments.map(p => p.id));
        activeJudgeAssignments.forEach(a => {
            if (projectsForAssignmentsIds.has(a.projectId)) {
                activeIds.add(a.judgeId);
            }
        });
        return activeIds;
    }, [projects, assignments, viewingLevel, overallHighestLevel]);
    ...
    const usersWithDetails = useMemo(() => {
        return filteredUsers.map(user => {
            ...
            for (const assignment of assignmentsForLevel) {
                ...
                if (project) { // should always be true from above filter
