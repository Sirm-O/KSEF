...
const BulkOperationsModal: React.FC<{ isOpen: boolean; onClose: () => void; }> = ({ isOpen, onClose }) => {
    const { user: currentUser, users, geographicalData, bulkCreateOrUpdateUsers } = useContext(AppContext);
    ...
    const headers = useMemo(() => {
        ...
        if (activeTab === 'judges') {
            return ['Name', 'Email', 'Role', ...optionalHeaders];
        }
        if (activeTab === 'admins') {
    ...
    const downloadTemplate = useCallback(() => {
        ...
        if (activeTab === 'judges') {
            exampleData = [
                'Dr. Jane Doe,jane.d@university.ac.ke,Judge,123456,',
                'Prof. John Smith,j.smith@tum.ac.ke,Coordinator,,23456789',
            ];
        } else if (activeTab === 'admins' && adminRoleToCreate && currentUser) {
            switch (adminRoleToCreate) {
                case UserRole.REGIONAL_ADMIN:
                    exampleData = ['Susan Njeri,susan.njeri@ksef.org,Central,,'];
                    break;
                case UserRole.COUNTY_ADMIN:
                    if (currentUser.currentRole === UserRole.REGIONAL_ADMIN) {
                        exampleData = ['Peter Kamau,peter.kamau@ksef.org,Kiambu,,'];
                    } else {
                        exampleData = ['Peter Kamau,peter.kamau@ksef.org,Central,Kiambu,,'];
                    }
                    break;
                case UserRole.SUB_COUNTY_ADMIN:
                    if (currentUser.currentRole === UserRole.COUNTY_ADMIN) {
                        exampleData = ['James Mwangi,james.mwangi@ksef.org,Gatundu South,,'];
                    } else {
                        exampleData = ['James Mwangi,james.mwangi@ksef.org,Central,Kiambu,Gatundu South,,'];
                    }
                    break;
            }
        }
    ...
    const parseCSV = useCallback((csvText: string) => {
        ...
        const results = lines.map((line): ParsedUser | null => {
            ...
            if (!name || !email) {
                ...
                if (existingUser) {
                    status = 'valid-update';
                    finalUser = { ...existingUser }; // Start with existing data
                } else {
                    status = 'valid-create';
                    finalUser = { name, email, roles: [], currentRole: UserRole.PATRON };
                }
    ...
    const handleConfirmAdd = async () => {
        ...
        await bulkCreateOrUpdateUsers(toCreate, toUpdate as User[]);
                ...
                {(activeTab === 'judges' || (activeTab === 'admins' && adminRoleToCreate)) && (
                    <>
                        ...
                        <div className="flex justify-end gap-4 pt-4">
                            ...
                            <Button type="button" onClick={handleConfirmAdd} disabled={validUsers.length === 0 || isLoading}>
                                {isLoading ? 'Processing...' : `Process ${validUsers.length} User(s)`}
                            </Button>
                        </div>
                    </>
                )}
