...
import { UserPlus, Edit, Trash2, Settings, Search, AlertCircle, Info, KeyRound, Users, Eye, FileDown, CheckCircle, Clock, Hourglass, MessageCircle } from 'lucide-react';
...
const AddEditUserModal: React.FC<{
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (userToEdit || formData.id) { // Check if we're editing an existing user
            ...
            const result = await addUserToList(userPayload as Omit<User, 'id'>, (status, progress) => {
                setCreationStatus(status);
                setCreationProgress(progress);
            });

            if (result && result.success) {
                setCreationStatus('Process Completed Successfully');
                    ...
                    {isSubmitting && creationStatus ? (
                        <div className="w-full space-y-2">
                            ...
                            <div className="w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700 overflow-hidden">
                                <div className="bg-primary h-2.5 rounded-full transition-all duration-500 ease-out" style={{ width: `${creationProgress}%` }}></div>
                            </div>
                        </div>
                    ) : (
                        <div className="flex justify-end gap-4">
                            <Button type="button" variant="ghost" onClick={onClose} disabled={isSubmitting}>Cancel</Button>
                            <Button type="submit" disabled={!!validationError || isSubmitting}>
